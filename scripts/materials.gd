...
## A see-through copy washed towards `tint`. Used for the piece the reader is holding in
## the room editor: a bed covers the box drawn under it, so the piece itself has to say
## whether it fits.
static func ghost_tint(src: Material, tint: Color, alpha := 0.6) -> Material:
	var key := "tint:%d:%s:%.2f" % [src.get_instance_id() if src != null else 0, tint.to_html(true), alpha]
	if _cache.has(key):
		return _cache[key]
	var m: BaseMaterial3D
	if src is BaseMaterial3D:
		m = src.duplicate()
		m.albedo_color = (src as BaseMaterial3D).albedo_color.lerp(tint, 0.7)
	else:
		# a custom shader cannot be made translucent, so it falls back to a plain pane
		m = StandardMaterial3D.new()
		m.albedo_color = tint
		m.roughness = 0.9
	m.albedo_color.a = alpha
